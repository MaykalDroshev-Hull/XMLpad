    ...
    public partial class MainWindow : Window
    {
        ...
        private void UIChanges(object sender, RoutedEventArgs e)
        {
            if (currentTheme == theme.Dark || (sender.Equals("firstBoot") && currentTheme == theme.Light))
            {
                ...
                App.Current.Resources["Menu.Static.Border"] = new SolidColorBrush(Colors.Black);
                ...
                App.Current.Resources["MenuItem.Highlight.Border"] = new SolidColorBrush(Colors.Black);
                ...
                foreach (var item in FindVisualChilds<MenuItem>(rightClickMenu))
                {
                    // Get the existing icon
                    var existingIcon = item.Icon as Image;

                    // If the existing icon is null or not an Image, skip this item
                    if (existingIcon == null)
                        continue;

                    // Replace the "darkMode" portion of the source string with "lightMode"
                    var newSource = existingIcon.Source.ToString().Replace("darkMode", "lightMode");

                    // Create a new Image with the updated source string
                    var newIcon = new Image { Source = new BitmapImage(new Uri(newSource)) };

                    // Set the new icon on the MenuItem
                    item.Icon = newIcon;
                }
            ...
            else
            {
                ...
                foreach(var item in FindVisualChilds<MenuItem>(rightClickMenu))
                {
                    ...
                    // Replace the "darkMode" portion of the source string with "lightMode"
                    var newSource = existingIcon.Source.ToString().Replace("lightMode", "darkMode");
